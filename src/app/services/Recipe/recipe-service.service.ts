...
export class RecipeServiceService {
  ...
  private baseUrl: 'http://localhost:' | undefined;
  authSubject: any;
  ...
  likeRecipe(recipe:any):Observable<any>{
    const headers=this.getHeaders();
    return this.http.put(`${this.baseUrl}/api/recipe/${recipe.id}/like`, recipe, {headers}).pipe()
    tap((updatedRecipe:any)=>{
      const currentState = this.recipeSubject.value;
      const updatedRecipes = currentState.recipes.map((item:any)=>item.id === updatedRecipe.id?updatedRecipe:item);
      this.recipeSubject.next({...currentState, recipes: updatedRecipes})
  ...
  deleteRecipe(id:any):Observable<any>{
    ...
    tap((deletedRecipe:any)=>{
      const currentState = this.recipeSubject.value;
      const deletedRecipes = currentState.recipes.filter((item:any)=>item.id == id);
